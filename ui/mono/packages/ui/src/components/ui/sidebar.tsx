...
function SidebarProvider({
  ...
  useEffect(() => {
    ...
    // b. Safely check the window size
    const handleResize = () => {
    ...
    // d. Cleanup function to remove listeners when component unmounts
    return () => {
      window.removeEventListener("resize", handleResize);
      window.removeEventListener("keydown", handleKeyDown);
    };
  }, []); // Empty dependency array ensures this runs only once on the client mount

  // 3. REVISED SETOPEN CALLBACK
  const setOpen = useCallback(
    (value) => {
      ...
      // Set cookie on the client when state changes
  ...
  // Derived state is now safe because it depends on safely-managed state
    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn(/* ...your classNames */ className)}
          {...props}
        >
          {children}
        </div>
