...
export function AssistantsPalette({ assistants }: { assistants: Assistant[] }) {
    const onDragStart = (event: React.DragEvent, assistant: Assistant) => {
        event.dataTransfer.setData("application/reactflow", JSON.stringify(assistant));
    ...
    return (
        <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">Drag to the workflow canvas</p>
            <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2">
                {assistants.length > 0 ? (
                    ...
                    <div className="text-sm text-muted-foreground text-center py-4">All assistants are on the canvas.</div>
                )}
            </div>
        </div>
    );
