...
import { AssistantNodeType } from "../nodes/AssistantNode";
...
export function useFlowLogic(
  ...
  const onNodesChange = useCallback(
    (changes: NodeChange<AssistantNodeType>[]) => setNodes((nds) => applyNodeChanges(changes, nds)),
    [setNodes]
  );
  ...
  const onDrop = useCallback(
    (event: React.DragEvent, assistant: Assistant | null) => {
      ...
      // 检查是否已存在
      const alreadyExists = nodes.some(n => n.data.assistantId === assistant.id);
      if (alreadyExists) {
