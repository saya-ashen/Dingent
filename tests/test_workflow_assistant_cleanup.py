...
from dingent.core.types import (
    AssistantCreate, WorkflowCreate, WorkflowNode, WorkflowEdge, 
...
def test_workflow_cleanup_on_assistant_deletion():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        workflow = workflow_manager.create_workflow(WorkflowCreate(
            name="test-workflow",
        ...
        node1 = WorkflowNode(
            ...
            data=WorkflowNodeData(
                assistantId=assistant1_id,
                assistantName=assistant1.name,
                isStart=True
            )
        )
        ...
        node2 = WorkflowNode(
            ...
            data=WorkflowNodeData(
                assistantId=assistant2_id,
                assistantName=assistant2.name,
                isStart=False
            )
        )
        ...
        deleted_refs = [
            node for node in updated_workflow.nodes 
            if node.data.assistantId == assistant1_id
        ]
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        workflow1.nodes = [
            WorkflowNode(
                ...
                type="assistant",
                position={"x": 100, "y": 100},
                data=WorkflowNodeData(assistantId=shared_id, assistantName="shared-assistant", isStart=True)
            ),
            WorkflowNode(
                id=str(uuid.uuid4()),
                type="assistant",
                position={"x": 300, "y": 100},
                data=WorkflowNodeData(assistantId=unique_id, assistantName="unique-assistant", isStart=False)
            )
        ]
