...
export function TableWidget({ data, viewMode }: { data: TablePayload, viewMode?: 'table' | 'card' }) {
    const { tableColumns, tableData } = useMemo(() => {
        if (!data || !data.columns || !data.rows) {
            return { tableColumns: [], tableData: [] };
        }

        const columns: ColumnDef<Record<string, any>>[] = data.columns.map(header => ({
            accessorKey: header,
            header: header,
            cell: ({ getValue }) => (
                <ReactMarkdown
        ...
        return { tableColumns: columns, tableData: data.rows };
    }, [data]);
