...
class WorkflowManager:
    ...
    def cleanup_workflows_for_deleted_assistant(self, assistant_id: str) -> list[str]:
        ...
        for workflow_id, workflow in self._workflows.items():
            ...
            # Remove the nodes
            workflow.nodes = [node for node in workflow.nodes if node.data.assistantId != assistant_id]

            # Remove edges connected to the removed nodes
            workflow.edges = [edge for edge in workflow.edges if edge.source not in node_ids_to_remove and edge.target not in node_ids_to_remove]
            ...
            logger.info(
                f"Cleaned up workflow '{workflow.name}' ({workflow_id}): "
