...
from backend.core.mcp_manager import get_async_mcp_manager, AsyncMCPManager
...
@app.get("/api/available_routes")
async def available_routes(lang: str = Depends(get_language)):
    available_routes = [{"value": "auto", "tools": [], "label": "自动选择"}]

    async with get_async_mcp_manager(settings.mcp_servers) as mcp:
        mcp_available_routes = await process_server(mcp, lang)

    available_routes.extend(mcp_available_routes)

    return {"available_routes": available_routes}


@app.get("/api/{server_name}/tips")
async def get_tips(server_name: str, lang: str = Depends(get_language)):
    async with get_async_mcp_manager(settings.mcp_servers) as mcp:
        ewas_client = mcp.active_clients.get(server_name)
        if not ewas_client:
            raise HTTPException(status_code=503, detail="EWAS MCP server not available")
        response = await ewas_client.read_resource(f"info://tips/{lang}")
