...
async def dynamic_authorizer(request: Request, user: User = Depends(get_current_user)):
    ...
    # Rule 2: Match /agent/{name} (execution)
        ...
        if user.role == "anonymous":
            raise HTTPException(
                status_code=403, 
                detail="Anonymous users are not allowed to execute agents."
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, 
                detail="Invalid JSON body for execution request."
            )
    ...
    # Rule 3: Match /action/{name} (action execution)
        ...
        if user.role == "anonymous":
            raise HTTPException(
                status_code=403, 
                detail="Anonymous users are not allowed to execute actions."
            )
    ...
    async def __call__(self, request: Request, user: User = Depends(get_current_user)):
        ...
        if "thread:owner" in self.required_permissions:
            ...
            if not thread_id:
                raise HTTPException(
                    status_code=400, 
                    detail="threadId is required for this operation."
                )
