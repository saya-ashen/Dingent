...
export function useMessagesManager() {
    ...
    // Helper function to check if a message is a valid ActionExecutionMessage for showing data
    const isShowDataActionMessage = (message: Message & { name?: string, arguments?: unknown }): message is ActionExecutionMessage => {
        // First, perform checks that narrow the type to ActionExecutionMessage
        if (message.type !== "ActionExecutionMessage" || message.name !== "show_data") {
            return false;
        }

        // Then, safely inspect the `arguments` property
        const args = message.arguments;
        if (typeof args !== 'object' || args === null || !('tool_output_id' in args)) {
            return false;
        }

        // Finally, check the type of the nested property
        const toolOutputId = (args as Record<string, unknown>).tool_output_id;
        return typeof toolOutputId === 'string' && toolOutputId.length > 0;
    };

    const widgetIdDependency = useMemo(() => {
        const allIds = messages
            .filter(isShowDataActionMessage)
            .flatMap((message) => message.arguments.tool_output_id as string);
    ...
    useEffect(() => {
        const fetchWidgetData = async () => {
            const dataMessages = messages.filter(isShowDataActionMessage);
